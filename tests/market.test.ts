...
import {bollingerBands,exponentialMovingAverage,movingAverage,normalizeBars,type Bar,type MarketSnapshot} from "../src/market";
...
test("history is limited to the requested real-bar window",()=>{
  const bars=Array.from({length:20},(_,index)=>({time:`2026-01-01T00:${String(index).padStart(2,"0")}:00Z`,open:100,high:102,low:99,close:101,volume:10}));
